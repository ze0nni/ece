...
{$IFDEF forth}
    ...
    // ‘ункци€ в зависимости от того, есть звездочка на конце слова или нет
    // ¬озвращает активный документ или документ, номер которого в вершине стека
...
begin
  ...
  if hPlugin = 0 then
    raise Exception.Create('Ќе удалось загрузить модуль ' + AFileName);

  LoadProc := GetProcAddress(hPlugin, 'GetPlugin');
  if @LoadProc = nil then
  begin
    FreeLibrary(hPlugin);
    raise Exception.Create('GetPlugin не найден в таблице экспорта модул€ ' +
        AFileName);
  end;
...
begin
  if (index < 0) or (index > DocumentsCount - 1) then
    raise Exception.Create('Ќеверный индекс документа');
...
begin
  ...
  if e.Count = 1 then
    //ѕервую строчку удал€ть ну ни как нельз€
    e.Strings[0] := ''
  else
...
procedure efSetEditorCaretLine(AMachine: IVForthMachine; AAthom: IVForthAthom;
  PAthomStr: PWideChar); stdcall;
begin
  { TODO -oOnni -cGeneral : Line только дл€ чтени€ }
