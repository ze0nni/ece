...
The Initial Developer of the Original Code is Sbastien Buysse [sbuysse@buypin.com]
...
Changes for the dxgettext project:

* removed dependencies on JVCL
* changed JVCL.INC to JEDI.INC
* renamed to SimpleXML
...
uses
  Windows, SysUtils, Classes{, IniFiles}{$IFDEF COMPILER6_UP}, Variants{$ENDIF};
...
{$ENDIF}
  ...
  private
    ...
    FEncodeDecode: boolean;
    procedure SetEncodeDecode(const Value: boolean);
  protected
  ...
  published
    ...
    property EncodeDecode: boolean read FEncodeDecode write SetEncodeDecode default true;
...
var
  DoDecode: procedure(var Value: string; TrimMultiple: Boolean = true) = nil;
  DoEncode: function(const Value: string): string = nil;

function NoEncode(const Value: string): string;
begin
  Result := Value;
end;

procedure NoDecode(var Value: string; TrimMultiple: Boolean = true);
begin
  // do nothing
end;
...
procedure TJvSimpleXml.SetEncodeDecode(const Value: boolean);
begin
  if FEncodeDecode <> Value then
  begin
    if Value then
    begin
      @DoDecode := @SimpleXMLDecode;
      @DoEncode := @SimpleXMLEncode;
    end
    else
    begin
      @DoDecode := @NoDecode;
...
begin
  if Pointer <> '' then
    result := Format(' %s:%s="%s"', [Pointer, Name, DoEncode(Value)])
  else
    result := Format(' %s="%s"', [Name, DoEncode(Value)]);
end;
{*************************************************}
...
begin
  ...
  begin
    if Value = '' then
      ...
      st := '>' + DoEncode(Value) + '</' + Name + '>' + sLineBreak;
    Stream.Write(st[1], Length(st));
  end
  else
  begin
...
begin
  if (Value <> '') then
  begin
    st := Level + DoEncode(Value) + sLineBreak;
    Stream.Write(st[1], Length(st));
  end;
  if Parent <> nil then
    Parent.DoSaveProgress;
